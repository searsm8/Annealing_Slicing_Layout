...
float REDUCTION_FACTOR = .99;  //factor that reduces annealing temperature
int     STEPS_PER_TEMP = 1000; //number of annealing steps performed at each temperature
...
vector <NET*> nets; //all nets connecting blocks in the design (never changes!)
...
vector <BLOCK*> next_blocks; //list of blocks for the NEXT move
...
class BLOCK
{
	...
	string name;
	string ops; //string of arbitrary length of the ops after this block
	int previous_ops_count; //count how many ops come prior to this one
	...
	bool cut_type; // false for H cut, true for V cut
	...
	{	
		...
		height = 0;
		area = 0;
		cut_type = false; // false for H cut, true for V cut
		max_AR = Si;
		min_AR = Ri;
...
stack <BLOCK*> save_state_stack; //used to keep the computations from previous
//calls of findBestArea()

bool save_state = false; //true when a stack has been saved
...
printf("Line 450\tblock: %s\tops: %s\tops.size(): %d\n", blocks[i]->name[0], blocks[i]->ops.c_str(), blocks[i]->ops.size());
	 //resolve all operators
	 	for(int op_index = 0; op_index < blocks[i]->ops.size(); op_index++)
		{
		printf("op_index: %d \t op: %s\n", op_index, blocks[i]->ops.at(1));
		continue;
			...
			if(blocks[i]->ops[op_index] == 'H') //read an op from the chain
				block_stack.push(combineBlocksHCut(b1, b2));
			else 	block_stack.push(combineBlocksVCut(b1, b2));
		
			//if the next move(s) are different in the layout string,
			//copy the stack as a "save state" to re-use the computations thus far.
			//if()
				//save_state_stack = block_stack;
...
//perhaps change the kind of moves based on temperature?
void performMove(float temp)
{	printf("Performing move1()!\n");
	switch(pickMove())
	{
		case 1: move1(); break;
		case 2: move2(); break;
...
void move2()
{
	...
	int i;
	do {
		i = ( rand() % (blocks.size()-1) ) + 1;
	} while(blocks[i]->ops.size() == 0);

	//complement the H's and V's
	string complement = "";
	for(int j = 0; j < blocks[i]->ops.size(); j++)
	{
		if(blocks[i]->ops[j] == 'H')
			complement += 'V';
		else complement += 'H';
...
void move3()
{
	...
	int attempts = 0; 		
	while(attempts < blocks.size()) //try this many times, then give up and cancel move
	{
		//choose a random op string until a non-empty one is found (never pick ops[0])
		int i;
		do {
		...
		if(blocks[i-1]->previous_ops_count < i-1) //check if this move would preserve balloting
		    { 	//check if this move would preserve normalization
			if(blocks[i-1]->ops.size() == 0 || blocks[i]->ops[0] != blocks[i-1]->ops[blocks[i-1]->ops.size()-1])
			{	//printf("moving backward!\n");
				blocks[i-1]->ops.push_back(blocks[i]->ops[0]);
				blocks[i]->ops.erase(blocks[i]->ops.begin());						
				blocks[i-1]->previous_ops_count++;				
				break;			
			}
		    } 
		
		
		if(i != blocks.size() - 1) //can't move the last op forward!
		{	//check if this move would preserve normalization		
			if(blocks[i]->ops[blocks[i]->ops.size()-1] != blocks[i+1]->ops[0]) 
			{
				blocks[i+1]->ops.insert(0, 1, blocks[i]->ops[blocks[i]->ops.size()-1]);
				blocks[i]->ops.erase(--blocks[i]->ops.end());						
				blocks[i]->previous_ops_count--;				
				break;		
			} 
		}
...
void update_previous_ops_count()
{
	blocks[0]->previous_ops_count = 0;
	
	for(int i = 1; i < blocks.size(); i++)	
	{
		blocks[i]->previous_ops_count = blocks[i-1]->previous_ops_count + blocks[i]->ops.size();
	}
}
...
void initializeOps()
{
	...
	blocks[0]->ops = ""; //first op must always be empty
	
	//initialize all other ops to "H"
	for(int i = 1; i < blocks.size(); i++)
	{
		blocks[i]->ops = "HVH";
	}
		
	update_previous_ops_count();	
	return;
}
...
float initializeTemp()
{
	...
	printf("Done with 10k delta moves!\n");
...
void deallocateBlock(BLOCK* b)
{
	...
	delete b; b = NULL; //call deconstructor
...
void anneal_slicing(string benchmark_name, int trial_num)
{
	...
	for(int i = 0; i < blocks.size(); i++)	
		printf("%s '%s' ", blocks[i]->name.c_str(), blocks[i]->ops.c_str());	
	printf("\n");	
